  ...
  <page-aside :showOnMobile="route().current('blocks.index')">
    <template v-slot:header>
      <jet-section-title>
        ...
        <template #aside
          ><button-link :href="route('blocks.create')"
            >New Block</button-link
          ></template
        >
      </jet-section-title>
    </template>
    ...
    <div class="bg-white">
      <ul class="divide-y divide-gray-200">
        <li v-for="block in blocks" :key="block.id" class="bg-white">
          <aside-link
            :href="route('blocks.show', block.id)"
            :current="
          ...
          >
            <div class="flex-1 min-w-0">
              <div class="focus:outline-none">
                ...
                <p v-if="block.city" class="text-sm text-gray-500 truncate">
                  {{ block.city
                  }}<span v-if="block.state">, {{ block.state }}</span>
                </p>
              </div>
            </div>
          </aside-link>
    ...
    <empty-state
      v-if="blocksLength < 1 && route().current('blocks.index')"
      heading="No Blocks"
