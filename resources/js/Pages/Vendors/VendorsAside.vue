  ...
  >
    ...
    <div class="bg-white">
      ...
      <ul class="divide-y divide-gray-200">
        <li v-for="vendor in filteredVendors" :key="vendor.id" class="bg-white">
          <aside-link
            :href="
              vendor.deleted_at
          ...
          >
            <div class="flex-1 min-w-0">
              <div class="focus:outline-none">
                ...
                <p v-if="vendor.state" class="text-sm text-gray-500 truncate">
                  {{ vendor.state }}
                </p>
              </div>
            </div>
    ...
    updateVendors() {
      this.filteredVendors = this.vendors.filter((vendor) => {
        if (
          ...
          vendor.state?.toLowerCase().includes(this.search.toLowerCase())
        ) {
          return true;
        } else {
          return false;
